...
from sqlalchemy import Boolean, Text, Date, String, Integer, Float, DateTime
# --- END SQLAlchemy Type Imports ---
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:shubham@localhost:5432/health_app_db'
...
class User(db.Model):
    ...
    role = db.Column(String(20), nullable=False, default='health_worker')
    audit_logs = db.relationship('AuditLog', backref='user', lazy=True)

class Patient(db.Model):
    __tablename__ = 'patients'
    id = db.Column(Integer, primary_key=True)
    patient_id = db.Column(String(20), nullable=False, index=True)
    ...
    bp_systolic = db.Column(Integer)
    bp_diastolic = db.Column(Integer)
    sugar = db.Column(Integer)
    height = db.Column(Float)
    weight = db.Column(Float)
    bmi = db.Column(Float)
    temp_status = db.Column(String(20))
    bp_status = db.Column(String(20))
    sugar_status = db.Column(String(20))
    bmi_status = db.Column(String(20))
    # Corrected data type to Date for consistency
    record_date = db.Column(Date)
    # Corrected method to datetime.now(timezone.utc)
    created_at = db.Column(DateTime, default=lambda: datetime.now(timezone.utc))
    other_vitals = db.Column(Text, nullable=True)
...
class Prescription(db.Model):
    ...
    reminder_type = db.Column(String(50), nullable=False, default='Medication')
    doctor_name = db.Column(String(100), nullable=True)
    medication_name = db.Column(String(150), nullable=True)
    frequency = db.Column(Integer, nullable=True)
    duration = db.Column(Integer, nullable=True)
    start_date = db.Column(Date, nullable=True)
    follow_up_datetime = db.Column(DateTime, nullable=True)
    special_note = db.Column(Text, nullable=True)
    # Corrected method to datetime.now(timezone.utc)
    created_at = db.Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
class Reminder(db.Model):
    ...
    id = db.Column(Integer, primary_key=True)
    prescription_id = db.Column(Integer, db.ForeignKey('prescriptions.id'), nullable=False)
...
class AuditLog(db.Model):
    ...
    event_type = db.Column(String(50))
    event_details = db.Column(Text)
    # Corrected method to datetime.now(timezone.utc)
    ...
    changed_from = db.Column(Text, nullable=True)
    changed_to = db.Column(Text, nullable=True)
# --- END MODELS ---
...
def create_user():
    if request.method == "POST":
        ...
        if new_password != confirm_password:
            ...
            new_user = User(username=new_username, password_hash=hashed_password, role=user_role)
            db.session.add(new_user)
            db.session.commit()
            log_audit_event('USER_CREATED', f"Admin created new user '{new_username}' with role '{user_role}'.")
            flash(f"User '{new_username}' created successfully!", "success")
            return redirect(url_for("dashboard"))
    return render_template("create_user.html", username=session.get("username"), user_role=session.get("role"))
